require_relative 'spellnames.rb'
...
step1.each {|step|
  step2 = step.split("\n")
  sphere_name = step2[0]
  unless sphere_name == nil
    sphere_name = sphere_name.downcase
  end

  spells_only = step2.count - 1
  step2.each {|step3|
    unless step3 == nil
      unless step3 == step2[0]
        step4 = step3.split(" (")
        
        step4.each_with_index {|step5, pos|
          if pos.even? == true
            File.write("spellnames.txt", "'#{step5}', ", mode: "a")
          else 
            step6 = step5[0]
            case step6
              when "1"
                first.push(step4[pos - 1])
              when "2"
                second.push(step4[pos - 1])
              when "3"
                third.push(step4[pos - 1])
              when "4"
                fourth.push(step4[pos - 1])
              when "5"
                fifth.push(step4[pos - 1])
              when "6"
                sixth.push(step4[pos - 1])
              when "7"
                seventh.push(step4[pos - 1])
              when "8"
                eighth.push(step4[pos - 1])
              when "9"
                ninth.push(step4[pos - 1])
            end
  ...
  sphere_template = "#{sphere_name}= { \n \t first: #{first}, \n \t second: #{second}, \n \t third: #{third}, \n \t fourth: #{fourth}, \n \t fifth: #{fifth}, \n \t sixth: #{sixth}, \n \t seventh: #{seventh}, \n \t eighth: #{eighth}, \n \t ninth: #{ninth} \n }"
      unless sphere_name == nil
        File.write("spheres.txt", "#{sphere_template} \n \n", mode: "a")
